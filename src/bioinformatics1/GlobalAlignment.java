...
public class GlobalAlignment {
    ...
    public int[][][] mk3DMatrix(char[] xs, char[] ys, int band) {
        band = adjustband(xs, ys, band);
        int[][][] result = initialize3Dmatrix(xs, ys, band);
        fill3DMatrix(xs, ys, result, band);
        return result;
    }

    private int adjustband(char[] xs, char[] ys, int band) {
        //if the band is -1 which means no banding, or the  band is too big
        if (band == -1 || band > xs.length + 1) {
            band = xs.length + 1;
        }
        if(band < 2){
            band = xs.length;
    ...
    private int[][][] initialize3Dmatrix(char[] xs, char[] ys, int band) {
...
//        result[U][0][0] =0;
    ...
    // How far to initialize? I don't want to go past the band width. LOL
        int imax = band < result[L].length ? band : result[L].length;
    ...
    public ArrayList<BacktraceStep> backtrace(int[][][] result) {
    ...
    private void fill3DMatrix(char[] xs, char[] ys, int[][][] result, int band) {
        LinearEquation diageq = utils.mkDiagEquation(result[0]);
        ...
        for (int i = 1; i < result[0].length; i++) {
            ...
            if(to >= result[0][0].length)
                to = result[0][0].length;
            for (int j = from; j < to; j++) {
                ...
                result[M][i][j] = Integer.max(Integer.max(
